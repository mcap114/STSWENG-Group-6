...
function deleteSelected() {
    ...
    const idsToDelete = Array.from(selectedCheckboxes).map(checkbox =>
        checkbox.closest('tr').getAttribute('data-benefactor-id')
    );
    ...
    fetch('/benefactors/delete-multiple', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        ...
        .then(response => {
            if (response.ok) {
                alert('Selected benefactors deleted successfully.');
            } else {
                alert('Failed to delete selected benefactors. Please try again.');
